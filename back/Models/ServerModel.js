...
export const getServerByInviteCodeService = async (inviteCode) => {
  const result = await pool.query("SELECT * FROM Servers WHERE inviteCode = $1", [inviteCode]);
  return result.rows[0];
};
...
export const deleteChannelByIdService = async (channelID) => {
  const result = await pool.query(`DELETE FROM channels WHERE id = $1`, [channelID]);
  return result.rowCount;
};
...
export const banUserFromServerService = async (serverId, userId, bannedBy, reason, expiresAt) => {
  ...
  const result = await pool.query(
    ...
    [serverId, userId, bannedBy, reason, expiresAt]
  );
  return result.rows[0];
};

export const checkUserBanService = async (serverId, userId) => {
  const result = await pool.query(
